...
def test_construct_version_info():
    ...
    # It does test that the __version__ attribute is set on hera_analysis.
    ...
    git_branch = subprocess.check_output(['git', 'rev-parse', '--abbrev-ref', 'HEAD'],
                                         stderr=subprocess.STDOUT).strip()
    git_version = subprocess.check_output(['git', 'describe', '--tags', '--abbrev=0', '--always']).strip()

    test_version_info = {'version': hera_analysis.__version__, 'git_origin': git_origin,
                         'git_hash': git_hash, 'git_description': git_description,
                         'git_branch': git_branch}

    nt.assert_equal(hera_analysis.version.construct_version_info(), test_version_info)

def test_main():
    version_info = hera_analysis.version.construct_version_info()

    saved_stdout = sys.stdout
    try:
        out = StringIO()
        sys.stdout = out
        hera_analysis.version.main()
        output = out.getvalue()
