...
def skynpz2calfits(fname, uv_file, dly_files=None, amp_files=None, bp_files=None, out_dir=None, phs_files=None, overwrite=False,
    ...
    """
    Turn casa calibration solutions in *.npz files (output from sky_image.py pipeline)
    into pyuvdata calfits file format.

    Paramters:
    ----------
    fname : type=str, output *.calfits filename

    uv_file : type=str, path to original miriad uv file that the solutions were derived from

    dly_files : type=str or list of str, path to npz file with antenna delay solutions

    amp_files : type=str or list of str, path to npz file with antenna amplitude solutions

    phs_files : type=str or list of str, path to npz file with antenna phase solutions

    bp_files : type=str or list of str, path to npz file with complex antenna bandpass solutions
    ...
    hc.abscal.gains2calfits(fname, gains, freqs, times, jones, overwrite=overwrite,
                            cal_style='sky',
                            sky_field=sky_field, sky_catalog=sky_catalog)
